...
class Trader:
    ...
    def hydrogel(self, state: TradingState, shared: dict):
        ...
        result      = []
        pos_lim     = 200
        quote_size  = 50     # per level
        half_spread = 4
        fv_window   = 20
        flat_thr    = 120    # quote both sides when |pos| < flat_thr
        unwind_thr  = 175    # cross spread when |pos| > unwind_thr
        ...
        if pos >= flat_thr:
            # long — only sell at two levels to unwind faster
            if sell_cap > 0:
            ...
            # flat — two-sided, two levels each side
            if buy_cap > 0:
                result.append(Order(product, bid_px,     min(quote_size, buy_cap)))
            if buy_cap > quote_size:
                result.append(Order(product, bid_px - 2, min(quote_size, buy_cap - quote_size)))
