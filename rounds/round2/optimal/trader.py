...
class Trader:
    ...
    def root(self, state: TradingState, shared: dict):
        ...
        # Sweep to full position limit 80 (baseline used 76)
        if pos < pos_lim and (best_ask <= first_ask + 2 or state.timestamp >= 2000):
            qty = min(pos_lim - pos, abs(order_depth.sell_orders[best_ask]))
            if qty > 0:
    ...
    def ash(self, state: TradingState, shared: dict):
        ...
        if fair_value is not None:
            ...
            # Active taking cap raised from ±40 to ±60: captures more mispricing
            for ask_px in asks:
